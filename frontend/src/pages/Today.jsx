...
import { Card, SectionTitle, CalorieRing, ProgressBar, Metric } from "../components/UI";
      ...
      {isCheckinDay && (
        <div onClick={onGoCheckin} style={{padding:"12px 16px",background:"rgba(0,229,160,0.08)",border:"1.5px solid rgba(0,229,160,0.3)",borderRadius:12,marginBottom:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            ...
            <div style={{fontSize:12,color:"var(--muted)",marginTop:2}}>Log your progress to unlock Milestone {currentWeek+1}</div>
          </div>
          <div style={{fontSize:12,fontWeight:700,color:"var(--accent)",whiteSpace:"nowrap",marginLeft:12}}>Go →</div>
        </div>
      )}
      ...
      <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:12,marginBottom:12}}>
        ...
        <Card className="card-accent-purple" style={{marginBottom:0}}>
          ...
          <div style={{fontSize:12,color:"var(--muted)"}}>
            ...
            <strong style={{color:"var(--accent)"}}>{p.intensity} kg</strong> this week
          </div>
        </Card>
      </div>
              ...
              <div style={{marginTop:12}}>
                <div style={{background:"var(--surface2)",borderRadius:8,padding:12,marginBottom:10,fontSize:13}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",paddingTop:8,fontWeight:700,color:"var(--accent)"}}>
                    <span>Total {result.usedAI?"(AI)":"(est.)"}</span>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                  ...
                  <div className="form-group"><label className="form-label">Calories</label><input className="form-input" type="number" value={mealCal} onChange={e=>setMealCal(e.target.value)} placeholder="kcal"/></div>
                </div>
