...
import {
  calcBMR, calcTDEE, calcBMI, calcBodyFat,
  dailyDelta, buildJourneyPlan, calcMacros, todayKey, MIN_CALORIES,
} from "../utils/fitness";
...
function defaultHabits(goal) {
  const base = [
    { id:1, label:"Drink 2.5L water",   done:false },
    { id:2, label:"7+ hours sleep",     done:false },
    { id:3, label:"Log all meals",      done:false },
  ];
...
export default function Setup() {
  ...
  const [form, setForm] = useState({
    ...
    activity:"1.55", goal:"fat_loss", targetWeight:"", intensity:"0.5",
  });
  ...
  const previewWeeks = showPreview
    ? Math.ceil(Math.abs(+form.weight - +form.targetWeight) / +form.intensity)
    : null;
  ...
  function handleBuild() {
    ...
    const age    = +form.age,    gender = form.gender;
    const height = +form.height, weight = +form.weight;
    const act    = +form.activity;
    const goal   = form.goal,    intensity = +form.intensity;
    ...
    const delta   = dailyDelta(intensity);
    const minCal  = MIN_CALORIES[gender];
    const base    = goal === "maintenance" ? tdee
      : goal === "fat_loss"    ? Math.max(minCal, tdee - delta)
      : tdee + delta;

    // Calculate daily macro targets from base calories
    const macros = calcMacros(base, goal);

    const { totalWeeks, weekTargets, milestoneWeights } = buildJourneyPlan({
      weight, targetWeight, intensity, goal, baseDailyTarget: base,
    });
    ...
    dispatch({
      ...
      payload: {
        ...
        journey: {
          ...
          macros,           // daily macro targets stored here
          totalWeeks, weekTargets, milestoneWeights,
          startWeight: weight, startDate: todayKey(), extensionWeeks: 0,
        },
        habits: defaultHabits(goal),
        meals: {}, checkins: [], currentWeek: 1,
      },
    });
  }
    ...
    <div className="page-content" style={{ maxWidth:540, margin:"0 auto" }}>
      <div className="card">
        <div className="setup-hero">
          ...
          <p>Set your goal — we'll calculate your calories, macros, and full week-by-week plan</p>
        </div>
          ...
          <>
            ...
            <div className="grid-2" style={{ marginBottom:12 }}>
              ...
              {form.goal !== "maintenance" && (
                <Input
                  label={form.goal === "fat_loss"
                    ? "Target weight — must be less than current (kg)"
                    : "Target weight — must be more than current (kg)"}
