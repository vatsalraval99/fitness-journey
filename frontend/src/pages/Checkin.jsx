...
import { calcCheckinTier, TIER_META, MIN_CALORIES } from "../utils/fitness";
...
export default function Checkin() {
  ...
  async function runCheckin() {
    ...
    if (!w||w<20||w>400) { alert("Enter a valid weight"); return; }
    setLoading(true);
    ...
    const extensionAdded = tier==="behind" && actual - mTarget < -0.6;
    const newMilestoneWeights = {};
    if (extensionAdded) {
      const newLast = j.totalWeeks + j.extensionWeeks + 1;
      newMilestoneWeights[newLast] = p.targetWeight;
    }
    ...
    try {
      aiText = await callClaude({
        system: "You are an adaptive fitness coach. Give warm, specific, data-driven coaching in 3–4 sentences. Factor in energy level and feeling.",
        userMessage: `Week ${currentWeek}. Goal: ${p.goal} at ${p.intensity}kg/week. Change: ${actual>=0?"-":"+"}${Math.abs(actual).toFixed(2)}kg (target: ${Math.abs(mTarget)}kg). Tier: ${tier}. Energy: ${energy}/5. Feeling: ${feeling}. Note: ${note||"none"}. New target: ${nextCal} kcal/day.`,
        maxTokens: 300,
      });
    } catch { /* use default */ }

    dispatch({ type:"COMPLETE_CHECKIN", payload:{ checkin, nextWeek, nextCalTarget:nextCal, extensionAdded, newMilestoneWeights } });
    setResult({ tier, actual, mTarget, aiText, nextCal, nextMilestone: j.milestoneWeights[nextWeek]||p.targetWeight, extensionAdded });
    setLoading(false);
    setWeight(""); setEnergy(0); setNote("");
    document.querySelectorAll(".star-btn").forEach(s=>{s.classList.remove("on");s.textContent="☆";});
  }

  const TIER_BADGE_COLOR = { on_track:"green", ahead:"green", slightly_ahead:"green", slightly_behind:"warn", behind:"danger" };
      ...
      <Card className="card-accent-purple">
        ...
        <div className="grid-2" style={{marginBottom:12}}>
          <div className="form-group">
            ...
            <input className="form-input" type="number" step="0.1" placeholder="79.5" value={weight} onChange={e=>setWeight(e.target.value)} />
          </div>
          <div className="form-group">
            <label className="form-label">Energy level this week</label>
            <div className="stars">
              {[1,2,3,4,5].map(n=>(
                <button key={n} className={`star-btn ${energy>=n?"on":""}`} onClick={()=>setEnergy(n)}>
                  {energy>=n?"★":"☆"}
                </button>
              ))}
            </div>
        ...
        <div className="form-group" style={{marginBottom:14}}>
          ...
          <textarea className="form-textarea" placeholder="e.g. Hit the gym 4 times, struggled with cravings on Friday…" value={note} onChange={e=>setNote(e.target.value)} style={{minHeight:52}} />
        </div>
        <Button variant="primary" fullWidth onClick={runCheckin} disabled={loading}>
          {loading ? "Analysing…" : "Evaluate Week & Build Next Milestone"}
        </Button>
      </Card>
      ...
      {result && (
        <Card>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",marginBottom:12}}>
            <Badge color={TIER_BADGE_COLOR[result.tier]||"warn"}>
            ...
            {result.extensionAdded && <Badge color="orange">Journey extended +1 week</Badge>}
          </div>
      ...
      <Card>
        ...
        {checkins.length===0 ? (
          ...
          <div key={c.week} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 0",borderBottom:"1px solid var(--border)",fontSize:13,gap:8,flexWrap:"wrap"}}>
            ...
            <Badge color={TIER_BADGE_COLOR[c.tier]||"warn"}>{TIER_META[c.tier]?.label}</Badge>
          </div>
        ))}
      </Card>
    </div>
